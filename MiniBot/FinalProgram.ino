...
do{
  ...
  float reverseoffset = accelerationFactor*(error*kp);
...
void reverse(float reverseoffset, float error){

float kpreverse = 0.0028;
float kdreverse = 0.002;
...
//deltaL = -12;
  ...
  deltaR = -1*(Delta + reverseoffset);
  deltaL = -1*(Delta - reverseoffset + forOffset);
  ...
  pulseL = (stopPulse + deltaR)*10;   // calculate length of pulse - flipped on purpose
  pulseR = (stopPulse + deltaL)*10;   // calculate length of pulse - flipped in purpose
  leftWheel.writeMicroseconds(pulseR);    // send pulse in microseconds
  rightWheel.writeMicroseconds(pulseL);
}
